...
def matchASM(machine,asm,Statecheck):
	...
	with open('./ASMs/issues/deprecated.json','rb') as f: 
		dep_apis = json.load(f)
	with open('./ASMs/issues/wrongcall.json','rb') as f: 
		wrong_apis = json.load(f)
	
	changes = []
	for key in machine:
		if machine[key]['label'] in dep_apis.keys():
			chan = {
				"pos": key,
				"dep_api": machine[key]['label'],
				"cor_api": dep_apis[machine[key]['label']],
				"type": 0			
			}
			changes.append(chan)
		if machine[key]['label'] in wrong_apis.keys():
			chan = {
				"pos": key,
				"dep_api": machine[key]['label'],
				"cor_api": wrong_apis[machine[key]['label']],
				"type": 1			
			}
			changes.append(chan)
								
	return Statecheck, changes
...
for asm in ASMs:
	...
	for k in asm.keys():
		Statecheck[str(k)] = 0
	Statecheck, changes = matchASM(machine,asm,Statecheck)
	del Statecheck[str(max([int(sp) for sp in Statecheck]))]
	if len(list(set(list(Statecheck.values())))) == 1:
		Statecheck = {x : 1 for x in Statecheck}	
		...
		if Violationlist[vobject][sv]==0:
			...
			if flag==0:
				...
				if True in findexit:
					...
					report+='Misusetype: Missing Method calls '+' : '+vasm[str(sv)]['label']+'\n'
				flag=1
			use=flag
	if use==0 and len(changes)==0:
		report+='No misuse\n'
	else:
		for c in changes:
			if c['type'] == 0:
				report += 'Misusetype: Deprecated APIs at pos ' + c['pos'] + ', use ' + c['cor_api'] + ' instead.\n'
			elif c['type'] == 1:
				report += 'Misusetype: Inefficient API call at pos ' + c['pos'] + ', use ' + c['cor_api'] + ' instead.\n'
report+='\n\n\n'
mfname = mfname.split('/')
rname = '/'.join(mfname[:-1])
